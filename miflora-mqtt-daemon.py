...
#Lista que guarda todos os registos dos sensores
LeituraDados = []
...
while True:
    for [flora_name, flora] in flores.items():
        ...
        data["registDay"] = dataAtual.strftime("%d/%m/%y")
        print_line('Result: {}'.format(json.dumps(data)))
        #Lista que guarda todos os registos dos sensores
        LeituraDados.append(data)
        if reporting_mode == 'mqtt-json':
    ...
    if daemon_enabled:
        ...
        break
    df = pd.read_json(json.dumps(LeituraDados), orient = 'records')
    if os.path.isfile('plants.csv'):
        df.to_csv('plants.csv',mode = 'a', index = False, header = False,)
    else :
        df.to_csv('plants.csv',mode = 'a', index = False, header = True,)
